...
$.ajax({type:"get",url:_b9.url,data:_b7,dataType:"json",success:function(_bd){//agang注释此处type应为get，回头可以考虑从外部传入
...
var val=data[name];
if (val == null) val="";//agang注释，解决IE6下面null显示为null的问题
//$("input[name="+name+"]",form).val(val);//解决加载Radio控件的问题
var radioBtn = $('input[name='+name+'][value='+val+'][type="radio"]' ,form);
...
function _43f(){
if (opts.url.indexOf('?')!=-1) opts.url=opts.url+'&_myid='+Math.random();
else opts.url=opts.url+'?_myid='+Math.random();
