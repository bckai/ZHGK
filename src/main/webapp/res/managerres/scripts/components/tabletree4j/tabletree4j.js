...
//TableTree4J Object
function TableTree4J(objectName,tableTree4JDir,n){
	//vars-------------------------------------------------------------------
	if (tableTree4JDir.indexOf("tabletree4j")!=-1) {this.tableTree4JDir=tableTree4JDir;}
	else {this.tableTree4JDir=tableTree4JDir+"/res/managerres/scripts/components/tabletree4j/"};
	this.obj=objectName;
	this.treeNodes=[];
	this.htmlCode="";
	this.rootNode;
	...
	this.icon ={
		...
		node:this.tableTree4JDir+'img/page'+n+'.jpg',
	...
	//show the tabletree to an html element,no header
	TableTree4J.prototype.printTableToElement=function(elementId){
		this.initNodes();
		var container=document.getElementById(elementId);
		//alert(this.htmlCode);
		container.innerHTML=this.tableDesc+this.htmlCode+"</table>";
		document.getElementById(this.obj+"Trid-1").parentNode.removeChild(document.getElementById(this.obj+"Trid-1"));
	};
	
	//show the tabletree to an html element,去掉root和根节点的图标
	TableTree4J.prototype.printMenuTreeToElement=function(elementId,menuId){
		this.initNodes();
		var container=document.getElementById(elementId);
		//alert(this.htmlCode);
		container.innerHTML=this.tableDesc+this.htmlCode+"</table>";
		this.openAllNodes();
		document.getElementById(this.obj+"ImgNod-1").parentNode.removeChild(document.getElementById(this.obj+"ImgNod-1"));
		//document.getElementById(this.obj+"Trid0").parentNode.removeChild(document.getElementById(this.obj+"Trid0"));
		document.getElementById(this.obj+"Trid"+menuId).parentNode.removeChild(document.getElementById(this.obj+"Trid"+menuId));

	}
	
	//show the tabletree to an html element,去掉root的图标
	TableTree4J.prototype.printDownTreeToElement=function(elementId){
		this.initNodes();
		var container=document.getElementById(elementId);
		//alert(this.htmlCode);
		container.innerHTML=this.tableDesc+this.htmlCode+"</table>";
	...
	TableTree4J.prototype.flowInitChildNodes=function(pnode){
		...
		if(nods.length>0){
			for(var i=0;i<nods.length;i++){
				...
				nods[i].htmlcode=this.codeNodeTR(nods[i])+"<td class=\""+this.config.treeStyle+"\"><div class='treenode_wrap_div'>"+nods[i].htmlcode;
...
//定义树,去掉根节点
var getDownTree = function(objName,myPath) {
	//==初始化
	var menuTree=new TableTree4J(objName,myPath);
	menuTree.toMenuMode();
	
	//==参数: rootName,id,booleanOpen,classStyle,hrefTip,hrefStatusText,icon,iconOpen	
	menuTree.setMenuRoot("",-1,false,"",null,null,null,null);
	return menuTree;
};

//定义导航菜单树,去掉根节点
var getMenuTree = function(objName,myPath,n) {
	//==初始化
	var menuTree=new TableTree4J(objName,myPath,n);
	menuTree.toMenuMode();
	
	//==参数: rootName,id,booleanOpen,classStyle,hrefTip,hrefStatusText,icon,iconOpen	
	menuTree.setMenuRoot("",-1,false,"",null,null,null,null);
	menuTree.config.useLine=false;
	menuTree.config.rootNodeBtn=false;
	return menuTree;
};

//定义表格树
var getTableTree = function(objName,myPath,colName,colWidth,tableWidth) {

	//==初始化
	var tableTree=new TableTree4J(objName,myPath);
	
	//==构造表格描述
	var title="";
	for (var li=0; li<colName.length; li++){
		title = title+"<th width='"+colWidth[li]+"'>"+colName[li]+"</th>"
	}
	tableTree.tableDesc="<table id='treetable' width='"+tableWidth+"' cellpadding='0' cellspacing='1' border='0'><thead><tr>"+title+"</tr></thead>";	
	tableTree.setHeader([],"-1",[],true,"");	
	//设置列样式
	tableTree.config.folderAutoUrl = true;
	tableTree.gridHeaderColStyleArray=new Array("","");
	tableTree.gridDataCloStyleArray=new Array("","");
	return tableTree;
};
