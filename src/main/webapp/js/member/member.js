...
function deleteUser(id) {
	 var url = ctxPath + '/rest/member/delete/' + id;
     if (confirm('确定要删除该条记录吗？')) {
     	$.post(url,function(data){
     		if(data.respCode=="success"){
     			alert('删除成功！');
     			location.href = ctxPath + '/rest/member/page/list';  
     		}else{
     			alert('删除失败！');
     		}
     });
...
function checkOnly(){
	...
	$.putJSON(ctxPath+'/rest/member/' + oldName + '/validator',$('#userForm').serialize(),function(resData){
...
function reSetPwd(id){
	confirm('确定要重置该用户密码？', function(r) {
		if(r){
			alert('密码重置成功！');
			location.href = ctxPath + '/rest/member/' + id+"/password/reset";
		}
	});
}
...
//		}
//	});
	$.post(ctxPath+'/rest/member/save',$('#userForm').serialize(),function(data){
		if(data.respCode=='success'){
			location.href = ctxPath + "/rest/member/page/list";
		}else{
...
function saveRoleAssign() {
	...
	// ==提交
	$.postJSON(ctxPath + "/rest/member/" + userId + "/role/assign", {
		roleIds : roleIds
	}, function(resData) {
		if (resData.status == "success") {
			alert('角色分配成功');
	...
	if($('#pwdForm').form('validate')){
		if($('#oldpassword').val() == $('#newpassword').val()){
			$.messager.confirm('提示信息', '您所输入的原始密码和新密码相同,您确定要继续“修改”？', function(r) {
				if(r) {
					$.putJSON(ctxPath+'/rest/member/password/update',$('#pwdForm').serialize(),function(resData){
						if(resData.status == "success"){
							alert('密码修改成功！');
						}else{
							alert('密码修改失败！');
...
function resetPWD(id){
	if(confirm("确定要重置密码？")){
		$.post(ctxPath + "/rest/member/"+id+"/password/reset",function(data){
			if(data.respCode=='success'){
				alert("系统密码已经重置");		
			}else{
				alert(data.respMessage);
...
function saveGroupAssign() {
	...
	// ==提交
	$.postJSON(ctxPath + "/rest/member/" + userId + "/group/assign", {
		groupIds : groupIds
	}, function(resData) {
		if (resData.status == "success") {
			alert('用户组分配成功');
