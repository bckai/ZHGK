 ...
 */
public class Vd_DongShiYuan extends HDBaseDriver {
	...
	static Logger log=LoggerFactory.getLogger(Vd_DongShiYuan.class);
	
	/**
	 * 初始化驱动并打开串口
	 * @param dvcId
	 */
	public Vd_DongShiYuan(String dvcId) {
	...
	public void update(Observable o, Object arg) {
		...
		System.out.println("接收数据来自：动视元车检器-" + this.dvcId + "-" + sr.spc.getPortName()
		 				+ "-" + CusDateTool.getSysTime().substring(0, 19) + "\r\n" + dataStr);
		data = dataStr.split(",");
		vdStruct=new VdStruct();
		vdStruct.setCarType(99);
		vdStruct.setDvcId(this.dvcId);
		vdStruct.setPeriod(Integer.parseInt(data[1]));
		vdStruct.setLane1Flux(Integer.parseInt(data[4]) + Integer.parseInt(data[6]) + Integer.parseInt(data[8]) + Integer.parseInt(data[10]));
		...
		vdStruct.setLane1Speed(Integer.parseInt(data[5]) + Integer.parseInt(data[7]) + Integer.parseInt(data[9]) + Integer.parseInt(data[11]) / 4);
		...
		vdStruct.setLane1Occ(Double.valueOf(data[35]));
		...
		vdStruct.setOccUp(vdStruct.getLane1Occ() + vdStruct.getLane2Occ());
		vdStruct.setOccDown(vdStruct.getLane3Occ() + vdStruct.getLane4Occ());
		vdStruct.setFluxUp(vdStruct.getLane1Flux() + vdStruct.getLane2Flux());
		vdStruct.setFluxDown(vdStruct.getLane3Flux() + vdStruct.getLane4Flux());
		vdStruct.setSpeedUp((vdStruct.getLane1Speed() + vdStruct.getLane2Speed()) / 2);
		vdStruct.setSpeedDown((vdStruct.getLane3Speed() + vdStruct.getLane4Speed()) / 2);
		vdStruct.setTransFlag(0);
		vdStruct.setLaneNum(Integer.parseInt("4"));
		vdStructService.save(vdStruct);
		// 将车检数据存储完成后，同时更新设备通讯状态为正常
		dsStruct = new DsStruct(dvcId,1,"通讯正常");
