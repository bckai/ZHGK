...
import com.hndfsj.driver.serailport.SerialPortConfig;
 ...
 */
public abstract class HDBaseDriver implements Observer {
	static Logger log=LoggerFactory.getLogger(HDBaseDriver.class);

	protected String dvcId;
	protected SerialPortConfig spc;
	protected SerialPortReader spr;
	...
	public void driverInit(String dvcId) {
		this.dvcId = dvcId;
		this.spc = new SerialPortConfig(this.dvcId);
		this.spr = new SerialPortReader(this.spc, this);
	}
	...
	/**
	 * 检查设备通讯链路连接状态 此方法由具体的设备驱动来实现，因为各种设备检查连接状态的命令与判断逻辑不同 update by yuanx
	 * 20160421 默认判定五分钟内有无通信，有则正常，反之故障 其它有特殊判定方式的设备驱动可重写此函数
	 */
